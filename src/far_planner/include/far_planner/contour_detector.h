...
struct ContourDetectParams {
    ...
    float voxel_dim;       // 地图分辨率
    float kRatio;          // 图像放大比例
    int kThredValue;       // 二值化阈值
    int kBlurSize;         // 模糊核的大小
    bool is_save_img;      // 是否保存图像
    std::string img_path;  // 保存位置
};

class ContourDetector {
private:
    Point3D odom_pos_;
    cv::Point2f free_odom_resized_;  // 机器人的位置在cv::Mat上的像素坐标
    ContourDetectParams cd_params_;
    PointCloudPtr new_corners_cloud_;             // 临时点云
    cv::Mat img_mat_;                             // 2D图像，就是从这里提取轮廓的
    std::size_t img_counter_;                     // 计数器，图像文件名
    std::vector<CVPointStack> refined_contours_;  // 输出结果  检测到的轮廓
    std::vector<cv::Vec4i> refined_hierarchy_;    // 轮廓层次关系
    NavNodePtr odom_node_ptr_;

    int MAT_SIZE, CMAT;           // 原始img_mat_尺寸以及中心点索引
    int MAT_RESIZE, CMAT_RESIZE;  // 放大后尺寸
    float DIST_LIMIT;             // 调节参数
    float ALIGN_ANGLE_COS;        // 余弦值 判定直线的阈值
    float VOXEL_DIM_INV;          // 分辨率的倒数
    ...
    inline bool IsIdxesInImg(int& row_idx, int& col_idx, const bool& is_resized_img = false) {
        const int max_size = is_resized_img ? MAT_RESIZE : MAT_SIZE;
        if (row_idx < 0 || row_idx > max_size - 1 || col_idx < 0 || col_idx > max_size - 1) return false;
        return true;
    }
    // 重置画布
    ...
    // 遍历OpenCV hierarchy 树，找到障碍物内部的轮廓
