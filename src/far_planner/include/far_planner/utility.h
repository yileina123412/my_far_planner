...
public:
    ...
    */
    static void ExtractNewObsPointCloud(
    ...
    static void CropPCLCloud(
        const PointCloudPtr& cloudIn, const PointCloudPtr& cloudCropped, const Point3D& centriod, const float& range);

    static void CropPCLCloud(const PointCloudPtr& cloudInOut, const Point3D& centriod, const float& range);

    static void CropBoxCloud(const PointCloudPtr& cloudInOut, const Point3D& center_p, const Point3D& crop_size);

    static void ClusterFilterCloud(const PointCloudPtr& cloudInOut, const float& radius, const std::size_t c_thred);
    ...
    static void ConvertCloudToPCL(const PointStack& point_stack, const PointCloudPtr& point_cloud_ptr);

    static void ConvertCTNodeStackToPCL(const CTNodeStack& ctnode_stack, const PointCloudPtr& point_cloud_ptr);
    ...
    static void TransformPoint3DFrame(const std::string& from_frame_id, const std::string& to_frame_id,
        const tf::TransformListener* tf_listener, Point3D& point);
    ...
    static std::size_t PointInXCounter(const Point3D& p, const float& radius, const PointKdTreePtr& KdTree);
    ...
    static void Flat3DPointCloud(
        const PointCloudPtr& cloudIn, const PointCloudPtr& cloudFlat, const bool& is_downsample = true);

    static void ExtractFreeAndObsCloud(
        const PointCloudPtr& newCloudIn, const PointCloudPtr& freeCloudOut, const PointCloudPtr& obsCloudOut);
    ...
    static float MarginAngleNoise(const float& dist, const float& max_dist, const float& angle_noise);
    ...
    static void InflateCloud(
        const PointCloudPtr& obsCloudInOut, const float& resol, const int& inflate_size, const bool& deep_z_inflate);
    ...
    static void RemoveOverlapCloud(
        const PointCloudPtr& cloudInOut, const PointCloudPtr& cloudRef, const bool& is_copy_cloud = false);

    static void RemoveIndicesFromCloud(const pcl::PointIndices::Ptr& outliers, const PointCloudPtr& cloudInOut);
    ...
    static void CreatePointsAroundCenter(const Point3D& center_p, const float& radius, const float& resol,
        PointStack& points_stack, const bool& is_sort = true);

    static Point3D RANSACPoisiton(
        const std::deque<Point3D>& pos_filter_stack, const float& margin, std::size_t& inlier_size);

    static PointPair RANSACSurfDirs(
        const std::deque<PointPair>& surf_dirs_stack, const float& margin, std::size_t& inlier_size);

    static float VerticalDistToLine2D(const Point3D& start_p, const Point3D& end_p, const Point3D& cur_p);

    static bool IsInCylinder(const Point3D& start_p, const Point3D& end_p, const Point3D& cur_p, const float& radius,
    ...
    static bool IsNodeInExtendMatchRange(const NavNodePtr& node_ptr) {
        if (FARUtil::IsPointInToleratedHeight(node_ptr->position, FARUtil::kTolerZ * 1.5f) &&
            (node_ptr->position - FARUtil::odom_pos).norm() < FARUtil::kSensorRange) {
            return true;
        }
        return false;
    }
    ...
    static bool PointInsideAPoly(const std::vector<Point>& poly, const Point& p) {
        ...
        for (i = 0, j = npol - 1; i < npol; j = i++) {
            ...
            if ((((vetex1.y <= p.y) && (p.y < vetex2.y)) || ((vetex2.y <= p.y) && (p.y < vetex1.y))) &&
